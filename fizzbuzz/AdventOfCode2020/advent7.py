...
input_strengt = '''wavy bronze bags contain 5 striped gold bags, 5 light tomato bags.
...
small = '''light red bags contain 1 bright white bag, 2 muted yellow bags.
...
input_list = input_strengt.splitlines()

cont_reg = r'(\d) (\w+ \w+) (bags|bag).'
...
class Bag:
    def __init__(self, name, contains):
        ...
        if contains == 'no other bags.':
            ...
            fa = re.findall(cont_reg, contains)
            self.bags = fa
    
    def __repr__(self):
        return 'Name: {}'.format(self.name)

one_bag = 'dark yellow bags contain 3 faded black bags, 2 dark salmon bags, 2 vibrant lavender bags.'
'posh red bags contain no other bags.'

pat = r'(\w* \w*) bags contain (.*)'        


bag_list = list()

for a_bag in input_list:
    mo = re.match(pat, a_bag)
    bag = Bag(mo.group(1), mo.group(2))
    bag_list.append(bag)

bag_dict = {bag.name: bag for bag in bag_list}

ueue = Queue(maxsize=0)
seen = set()
ueue.put('shiny gold')


#filtered = list(filter(lambda x: 'shiny gold' in x.contains, bag_list))
...
while True:
    ...
    filtered = list(filter(lambda x: item in x.contains, bag_list))

    for bag in filtered:
        if bag.name not in seen:
            seen.add(bag.name)
            ueue.put(bag.name)
...
def findall_bags(bag_name):
    ...
    return_value = 1
    for b in bag.bags:
        return_value += int(b[0]) * findall_bags(b[1])
    return return_value

sg = findall_bags('shiny gold')
print(sg - 1)
