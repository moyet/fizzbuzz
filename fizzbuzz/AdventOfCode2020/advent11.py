strengt = '''LLLLLL.LLLLLLLLLLLL.LLLLLL.LLLLLLLLLLLL.LLLLLLLLLLLLL.LLLLLLLLLLLLLLLLLLLLLLLLL.LLLLLLLL.LLLLLLLLL
...
lines = strengt.splitlines()
for line in lines:
    imamatrix.append([l for l in line])

def pretty_print(matrice):
    for row in matrice:
        print("".join(row))

size = (len(imamatrix), len(imamatrix[0]))

def find_a_seat(matrice):
    def check_neighboors(i,j):
        occupied = 0
    ...
    new_matrix = [row.copy() for row in matrice]
    ...
    for i in range(size[0]):
        for j in range(size[1]):
            if matrice[i][j] == '.':
                ...
                if check_neighboors(i,j) == 0:
                    new_matrix[i][j] = '#'
            elif matrice[i][j] == '#':
                if check_neighboors(i,j) > 3:
                    new_matrix[i][j] = 'L'

    return new_matrix

def question_one(imamatrix):
    for i in range(100):
        pretty_print(imamatrix)
        print("\n\n\n")
        new_matrix = find_a_seat(imamatrix)
        if new_matrix == imamatrix:
            print('we are the same: ', i)
            break
        imamatrix = new_matrix
...
def find_a_seat2(matrice):
    def check_direction(i, j, direction):
        new_field = (i + direction[0], j + direction[1])
        if new_field[0] not in range(size[0]) or new_field[1] not in range(size[1]):
            return False
        felt = matrice[i + direction[0]][j + direction[1]]
        if felt == '.':
            return check_direction(i + direction[0], j + direction[1], direction)
        elif felt == '#':
            return True
        else:
            return False
    ...
    for i in range(size[0]):
        for j in range(size[1]):
            if matrice[i][j] == '.':
                ...
                if check_neighboors(i,j) > 4:
                    new_matrix[i][j] = 'L'

    return new_matrix

def question_two(matrice):
    for i in range(100):
        pretty_print(matrice)
        print("\n\n\n")
        new_matrix = find_a_seat2(matrice)
        if new_matrix == matrice:
            print('we are the same: ', i)
            break
        matrice = new_matrix
